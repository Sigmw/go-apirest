...
func (repositorio Usuarios) Criar(usuario modelos.Usuario) (uint64, error) {
	repositorio.db.Exec(`CREATE TABLE usuarios (
	...
	`)
	usuario.Cargo = 0          //garantir que vai cadastrar como usuario
	usuario.Desativado = false //garantir que nao vai desativar conta

	statement, erro := repositorio.db.Prepare(
		"insert into usuarios (nome, nick, email, senha, cargo, desativado) values(?, ?, ?, ?, ?, ?)",
	)

	userhabbo, err := habbo.GetUser(usuario.Nick)
	if err != nil {
		return 0, err
	}

	if usuario.Codigo == "" || usuario.Codigo != userhabbo.Motto {
		fmt.Println(usuario.Codigo)
		return 0, errors.New("Codigo invalido!")
	}
	...
	resultado, erro := statement.Exec(usuario.Nome, usuario.Nick, usuario.Email, usuario.Senha, usuario.Cargo, usuario.Desativado)
	if erro != nil {
		return 0, erro
	}
